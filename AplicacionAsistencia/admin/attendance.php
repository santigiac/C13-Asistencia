<?php
/**
 * Pasar Lista - Registro de asistencia
 */
...
$selectedGroupId = $_GET['group_id'] ?? ($_POST['group_id'] ?? '');
...
$existingAttendance = [];
if ($selectedGroupId) {
    $existingAttendance = $attendance->getByGroupAndDate($selectedGroupId, $selectedDate);
    // Si no hay resultados de attendance query, cargar alumnos directamente
    if (empty($existingAttendance)) {
        $rawStudents = $studentManager->getByGroup($selectedGroupId);
    ...
    <div class="app-layout">
        ...
        <main class="main-content">
            <div class="page-title fade-in">
                <h1>✅ Pasar Lista</h1>
                <p>Registra la asistencia de los alumnos</p>
            </div>
                            ...
                            <select name="group_id" class="form-select" onchange="this.form.submit()">
                                ...
                                <?php foreach ($groups as $g): ?>
                                    <option value="<?= $g['id'] ?>" <?= $selectedGroupId == $g['id'] ? 'selected' : '' ?>>
                                        <?= htmlspecialchars($g['name']) ?> (<?= $g['student_count'] ?> alumnos)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
            ...
            <!-- Lista de alumnos -->
            <?php if ($selectedGroupId && !empty($existingAttendance)): ?>
                ...
                <div class="card fade-in">
                    <div class="card-body">
                        <div class="empty-state">
                            ...
                            <p>Añade alumnos al grupo desde la sección de Alumnos.</p>
                            <a href="<?= BASE_URL ?>/admin/students.php?action=new" class="btn btn-primary mt-2">
                                ➕ Añadir Alumno
                            </a>
                        </div>
                    </div>
                ...
                <div class="card fade-in">
                    <div class="card-body">
                        <div class="empty-state">
                            ...
                            <p>Elige un grupo y una fecha para empezar a pasar lista.</p>
                        </div>
                    </div>
                </div>
            <?php endif; ?>
